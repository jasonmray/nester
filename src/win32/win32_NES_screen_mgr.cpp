...
boolean win32_NES_screen_mgr::GoWindowed()
{
  ...
  if(screen)
  {
    ...
    if(screen->lock(src))
    {
      if(sm->lock(dest))
      {
        for(uint32 y = 0; y < MIN(dest.height, src.height); y++)
        {
          memcpy(&dest.data[dest.pitch*y],
                 &src.data[src.pitch*y],
                 MIN(dest.width, src.width));
        }
...
boolean win32_NES_screen_mgr::GoFullscreen()
{
  ...
  {
    {
      win32_windowed_NES_screen_mgr* temp = (win32_windowed_NES_screen_mgr*)screen;
      if(temp->isBlownUp())
      {
        temp->shrinkAgain();
      }
